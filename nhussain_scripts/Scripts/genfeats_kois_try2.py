...
#print test_KOIS.shape
test_KOIS = pd.read_csv("../../Training/catalogs/shortperiodbright_KOI.csv")
#test_KOIS.head()


file_name = test_KOIS["kepid"] 
total_filenames= []
#add in all the time 26 day segments
for files in file_name:
    for i in range(21):
        total_filenames.append(str(files) + "-"+str(i)+ "_prim_ltf.lc")
...
for name in total_filenames:
    ...
    try:
        first_bls = np.loadtxt("../../Training/simulation/primaryKOI_try2/" + name[:-2]+"blsanal")[0, 1:]
        #10397751-4_prim_ltf.blsanal
        first_lc = np.loadtxt("../../Training/simulation/primaryKOI_try2/"+name)
        print name
    except:
        print "Faulty file", name
        continue 
    #print name[: -3]+"blsanal", name[:-4]+".txt" 
    ...
    np.savetxt("../KOI_ModShiftVals_Try2/" + name[:-7] +"_feats_fixed.txt", total_Vals, header=head_txt)
    #break
