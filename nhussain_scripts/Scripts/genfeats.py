...
for name in data_names[:10000]:#[20000:21000]:
    first_bls = np.loadtxt("../Training/simulation/injection/" + name)[0, 1:]
    first_lc = np.loadtxt("../Training/simulation/injection/"+name[:-8]+".txt")
    ...
    np.savetxt("InjModShiftVals/" + name[:-8] +"_feats_fixed.txt", total_Vals, header=head_txt)
    #break
