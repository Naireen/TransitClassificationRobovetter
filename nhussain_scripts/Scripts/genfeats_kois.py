...
for name in total_filenames:
    ...
    try:
        first_bls = np.loadtxt("../Training/simulation/primaryKOI/" + name[:-2]+"blsanal")[0, 1:]
        #10397751-4_prim_ltf.blsanal
        first_lc = np.loadtxt("../Training/simulation/primaryKOI/"+name)
        print name
    except:
    ...
    np.savetxt("KOI_ModShiftVals/" + name[:-7] +"_feats_fixed.txt", total_Vals, header=head_txt)
    #break
